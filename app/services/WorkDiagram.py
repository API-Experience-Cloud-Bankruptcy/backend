import httpx
from app.models.work_diagram import WorkProject, WorkProjectResponse
...
async def fetch_work_projects() -> list[WorkProject]:
    async with httpx.AsyncClient() as client:
        response = await client.get(API_URL)
        response.raise_for_status()
        data = response.json()
        work_project_response = WorkProjectResponse.model_validate(data)
        return work_project_response.result.results
